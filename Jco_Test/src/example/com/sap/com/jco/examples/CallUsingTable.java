...
public class CallUsingTable
{
    ...
    public static void main(String[] args) throws JCoException
    {
        ...
        JCoFunction function = destination.getRepository().getFunction("BAPI_COMPANYCODE_GETLIST"); //1. SAP 함수 객체에 저장
        if (function  ==  null)
            throw new RuntimeException("BAPI_COMPANYCODE_GETLIST not found in SAP.");

        try
        {
            function.execute(destination); //2. SAP 함수 실행
        }
        ...
        JCoTable codes = function.getTableParameterList().getTable("COMPANYCODE_LIST"); //3. COMPANYCODE_LIST 매개변수의 값 저장
